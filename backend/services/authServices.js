...
const { criarUsuarioRepository, buscarUsuarioPorEmail, buscarUsuarioPorId } = require('../repository/userRepository');

const registerUser = async (name, email, password) => {
    const hashedPassword = await hashPassword(password);
    const user = await criarUsuarioRepository(name, email, hashedPassword);
    return user;
};
